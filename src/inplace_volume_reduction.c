...
void collapse_edges(struct surface *surf, double **normals, double D2, double V, double H, size_t *vert_map, int *v_to_remove, int *f_to_remove, size_t n_vertices, size_t n_faces);
...
void dk_volume_reduction(struct surface *surf, double ***normals, double D2, double V, double H){
    ...
    size_t vertices_map[n_vertices];
    int vertices_to_remove[n_vertices];
    ...
    collapse_edges(surf, *normals, D2, V, H, &(vertices_map[0]), &(vertices_to_remove[0]), &(faces_to_remove[0]), n_vertices, n_faces);
...
void collapse_edges(struct surface *surf, double **normals, double D2, double V, double H, size_t *vert_map, int *v_to_remove, int *f_to_remove, size_t n_vertices, size_t n_faces){
    ...
    for (i = 0; i < n_faces; i++){
        ...
        if (d01 < D2){
            vert_map[v_id0] = v_id1;
            v_to_remove[v_id0] = 1;
            f_to_remove[i] = 1;

            AVGPT(v1, v0, v1);        
            
            AVGPT(normals[v_id1], normals[v_id0], normals[v_id1]);
            continue;
        }
        ...
        if (d12 < D2){
            vert_map[v_id1] = v_id2;
            v_to_remove[v_id1] = 1;
            f_to_remove[i] = 1;

            AVGPT(v2, v1, v2);        
            
            AVGPT(normals[v_id2], normals[v_id1], normals[v_id2]);
        ...
        if (d20 < D2){
            vert_map[v_id0] = v_id2;
            v_to_remove[v_id0] = 1;
            f_to_remove[i] = 1;

            AVGPT(v2, v0, v2);        
            
            AVGPT(normals[v_id2], normals[v_id0], normals[v_id2]);
        }
    }
}
